...
void main() {
  ...
  /// Skips loudly on hosts that cannot apply profiles (dev cubes).
  void needKernel() {
    if (hostGuard != null) markTestSkipped(hostGuard);
  }
...
''');
  ...
  test('E2E-1/AC1: launcher exits 0 while the harness is still running', () async {
    needKernel();
    final mark = '$proj/mark1.txt';
    writeProfile('spawn1', [script('h1.sh', 'sleep 4\nprintf done > "\$MARK"\n')]);
    ...
    expect(errOut, contains('under cube-sandbox'), reason: 'banner before exit');
    expect(
      File(mark).existsSync(),
      isFalse,
      reason: 'harness (sleep 4) must outlive the launcher',
    );
    expect(awaitFile(mark), isTrue, reason: 'harness finishes after cube died');
    expect(File(mark).readAsStringSync(), 'done');
  });

  test('E2E-2/AC2: launch exit = spawn status (0 even at harness 7); 126 fail-closed', () {
    needKernel();
    ...
    expect(r7.exit, 0, reason: 'spawn succeeded — harness exit 7 is not ours');
  ...
  test('E2E-3/AC3+F1: confinement survives the launcher; unconfined control writes', () async {
    needKernel();
    ...
    expect(awaitFile('$proj/done3.txt'), isTrue, reason: 'harness finished after cube died');
  ...
  test('E2E-4/AC4+F2: harness output reaches the terminal after cube exits', () async {
    needKernel();
    writeProfile('spawn4', [script('h4.sh', 'sleep 2\necho after-parent-death')]);
    ...
    expect(
      ...
      reason: 'bytes written after the launcher died reach the inherited stdout',
    );
  ...
  test('E2E-5/AC5+F3: SIGINT reaches the harness after cube exits (foreign-group control)', () async {
    needKernel();
    final perl = '/usr/bin/perl';
    if (!File(perl).existsSync()) {
      markTestSkipped('/usr/bin/perl missing — cannot isolate a process group');
    }
    ...
    Future<Process> spawnIsolated(String profile, String markDir) =>
        Process.start(perl, [
          ...
          'launch',
          profile,
        ], workingDirectory: proj, environment: env({'MARKDIR': markDir}));
    ...
    expect(awaitFile('$proj/a5/DONE'), isTrue, reason: 'trap survived, script continued');
    ...
    expect(kill2.exitCode, 0, reason: 'signal fired at the foreign group only');
    expect(awaitFile('$proj/b5/DONE'), isTrue, reason: 'arm-2 harness finished naturally');
    expect(
  ...
  test('E2E-6/AC7: --wait forwards the child exit verbatim (0 / 7 / 130)', () {
    needKernel();
    writeProfile('spawn6', [argvSh]);
    expect(
      h
          .launchCubeSandbox(
            ['launch', 'spawn6', '__exit', '0', '--wait'],
            cwd: proj,
            env: env(),
          )
          .exit,
  ...
  test('E2E-7/AC6+F4: the orphaned harness reparents to launchd (ppid 1)', () async {
    needKernel();
    writeProfile('spawn7', [
  ...
  test('IT-1/AC8: staged SBPL byte-identical across default, --wait and sbpl', () {
    needKernel();
    writeProfile('spawn8', [argvSh]);
    final sbpl = h.launchCubeSandbox(['sbpl', 'spawn8'], cwd: proj, env: env());
    ...
    final staged = Directory(
      '$proj/.cube-sandbox/cache',
    ).listSync().whereType<File>().where((f) => f.path.endsWith('.sb')).toList();
    expect(staged, hasLength(1), reason: 'mode is not an emit input — same key10');
  ...
  test('IT-2/AC9: usage and README document spawn-and-exit and --wait', () {
    ...
    expect(text, contains('spawn-and-exit'));
